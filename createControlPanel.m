...
paraExplanation = {sprintf('%s\n%s\n%s', 'This parameter is a threshold factor used for nuclei segmentation by Otsu¡¯s method.', 'Low: Segment more nuclei', 'High:Segment fewer  nuclei'),...
    sprintf('%s\n%s\n%s', 'This parameter is a second threshold factor used to adjust the previous segmentation.', 'Low: Segment more individual nuclei', 'High:Segment fewer individual nuclei'),...
    sprintf('%s\n%s\n%s\n%s', 'This parameter specifies the radius of the disk used during morphological opening to separate weakly connected nucleus objects', 'Notice: this parameter must be a positive integer.', 'Low: Fewer nuclei are separated', 'High:More nuclei are separated'),...
    sprintf('%s\n%s\n%s\n%s', 'This parameter is a threshold that separates clustered nuclei from non-clustered nuclei', 'Notice: this parameter must be positive and  < 1 .', 'Low: More objects are identified as individual nuclei ', 'High:More objects are identified as nuclei clusters'),...
    ...
    sprintf('%s\n%s\n%s', 'This parameter sets the minimum size of a single nucleus.', 'Low: More nuclei would be regarded as too small to be a true nucleus', 'High:Fewer nuclei would be considered  as too small to be a true nucleus'),...
    sprintf('%s\n%s\n%s', 'This parameter is a threshold factor used for cell bodies and neurites segmentation by Otsu¡¯s method.', 'Low: Include more pixels as parts of cell bodies and neurites', 'High:Include fewer pixels as parts of cell bodies and neurites'),...
    sprintf('%s\n%s\n%s\n%s', 'This parameter controls the size of the disk used to isolate cell bodies from neurites.', 'Notice: this parameter must be a positive integer.', 'Low: Remove fewer neurites ', 'High:Remove more neurites'),...
    sprintf('%s\n%s\n%s', 'This parameter is a threshold factor used solely for neurites segmentation by Otsu¡¯s method.','Low: Include more pixels as parts of neurites', 'High:Include fewer pixels as parts of neurites'),...
    sprintf('%s\n%s\n%s', 'This parameter is a second threshold factor used to adjust the previous segmentation.', 'Low: Capture more additional neurites', 'High:Capture fewer additional neurites'),...
    sprintf('%s\n%s\n%s\n%s', 'This parameter indicates the side length of a square used to join nearby neurite sections by morphological closing.',  'Notice: this parameter must be a positive integer.', 'Low: Merge fewer neurites into a single section', 'High:Merge more neurites into a single section'),...
    };    
