...
% Look for unused or overused incomplete edges
for i = 1:numel(incompleteEdgesBranch)
    iceb = incompleteEdgesBranch{i};
    for j = 1:numel(iceb)
        fprintf('i=%d of %d  j=%d of %d\n', i, numel(incompleteEdgesBranch), j, numel(iceb));
        ice = iceb{j};
        assert(ice.useCount == 1, 'Misused branch edge i=%d  j=%d', i, j)
    end
end


unusedSingleVertexCount = 0;
for i = 1:numel(incompleteEdgesPath)
    iceb = incompleteEdgesPath{i};
    for j = 1:numel(iceb)
        fprintf('i=%d of %d  j=%d of %d\n', i, numel(incompleteEdgesPath), j, numel(iceb));
        ice = iceb{j};
        if ice.useCount == 1
            assert(ice.vertices(1) > 0 || ice.vertices(2) > 0, 'Path edge should not have been used once i=%d j=%d vertices=[%d %d]', i, j, ice.vertices(1), ice.vertices(2));
        end
        if ice.useCount == 2
            assert(ice.vertices(1) == 0 && ice.vertices(2) == 0, 'Path edge should not have been used twice i=%d j=%d vertices=[%d %d]', i, j, ice.vertices(1), ice.vertices(2));
        end
        if ~(ice.useCount == 1 || ice.useCount == 2)
            if numel(ice.pathIdxList) == 1 && ice.vertices(1) > 0 && ice.vertices(1) == ice.vertices(2) && ice.useCount == 0
                fprintf('Unused single vertex path at %d\n', ice.pathIdxList(1));
                unusedSingleVertexCount = unusedSingleVertexCount + 1;
            else
                error('Unused path edge i=%d  j=%d  vertices=[%d %d]', i, j, ice.vertices(1), ice.vertices(2));
            end
        end
    end
end
fprintf('Found %d unused single vertex edges\n', unusedSingleVertexCount);




% Assign labels to incomplete edges in branch point neighborhoods
