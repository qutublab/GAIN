...
function [edges vertexLocations] = findEdges2(skeleton, branchPoints, endPoints)
...
% Always use smaller vertex number as first index and larger vertex number as
% second index
...
% Incomplete branch edges have the negation of the next path location stored
% in the second element of the vertices field of the edge
...
for i = 1:numel(incompleteEdgesBranch)
    for j = 1:numel(incompleteEdgesBranch{i})
        if ~isempty(incompleteEdgesBranch{i}{j})
            error('Incomplete branch edge at i=%d j=%d', i, j)
        end
    end
end
...
for i = 1:numel(incompleteEdgesPath)
    for j = 1:numel(incompleteEdgesPath{i})
        if ~isempty(incompleteEdgesPath{i}{j})
            error('Incomplete path edge at i=%d j=%d', i, j)
        end
    end
end
...
for i = 1:edgeIdx
    e = edges{i};
    if e.vertices(1) <= 0 || e.vertices(2) <= 0
        error('Edge %d does not terminate at vertices (%d, %d)', e.idNum, e.vertices(1), e.vertices(2));
    end
end
