...
for i = 1:numel(pathIdxList)
    ...
    % Check for adjacency of consecutive indices
    [R C] = ind2sub(cc.ImageSize, pathIndices);
    r1 = R(1);
    c1 = C(1);
    for j = 2:numel(R)
        r2 = R(j);
        c2 = C(j);
        diff = abs(r1 - r2) + abs(c1 - c2);
        if diff < 1 | diff > 2
            error('i=%d j=%d  r1=%d c1=%d  r2=%d c2=%d', i, j, r1, c1, r2, c2)
        end
        r1 = r2;
        c1 = c2;
    end
