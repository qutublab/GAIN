...
%             fullPathPrefix = strcat(outputDir, filesep, prefix);
        ...
        function status = firstNucleusSegmentation(nip)
            ...
            thresh1 = min(1, thresh1 * nip.parameters.SegmentBrightNuclei);
        ...
        function status = secondNucleusSegmentation(nip)
            ...
            thresh2 = min(1, thresh2 * nip.parameters.SegmentDimNuclei);
        ...
        function status = openNucleusMask(nip)
            status = '';
            se = strel('disk', nip.parameters.SeparateWeaklyConnectedNuclei , 0);
...
%             figure, imshow(nip.openedNucleusMask,'initialmagnification','fit');
            ...
            for l = numLabels:-1:1
                ...
                cluster = solidity < nip.parameters.DetermineNucleiClusters;
                if ~cluster
...
%             fprintf('[NeuronImageProcessor.identifyNucleusClusters] minSolidity=%f  maxSolidity=%f (%d objects)\n', minSolidity, maxSolidity, numLabels);
            ...
            if (~nip.optimization) && (nonclusterCount == 0)
                status = sprintf('No non-clustered nuclei were found at an DetermineNucleiClusters of %f',...
                   nip.parameters.DetermineNucleiClusters); 
            end
        end
        ...
        function status = calculateNominalMeanNucleusArea(nip)
            ...
            if numel(singleNucleusArea) == 0
                ...
                nip.nominalMeanNucleusArea = nip.medianSingleNucleusArea * nip.parameters.SingleNucleusSize;
            end
...
%                end
        ...
        function status = calculateMinNucleusArea(nip)
            status = '';
            nip.minNucleusArea = max(1, ceil(nip.medianSingleNucleusArea / nip.parameters.MinAcceptableNucleusSize ));
            for i = 1:numel(nip.nucleusDataArr)
...
%end
        ...
        function status = segmentCellBodies(nip)
            ...
            thresh1 = min(1, thresh1 * nip.parameters.CellBodiesandNeurites);
...
%             nip.firstCellMask = imfill(nip.firstCellMask, 'holes');
        ...
        function status = isolateCellBodies(nip)
            status = '';
            se = strel('disk', nip.parameters.RemoveNeurites, 0);
        ...
        function status = resegmentNeurites(nip)
            ...
            thresh2 = min(1, thresh2 * nip.parameters.SegmentNeurites);
...
%            gmagThresh1 = min(1, graythresh(nip.gradientMag) * nip.parameters.SegmentNeurites);
...
%             edges1 = edges1 & ~nip.openedCellBodyMask;
            ...
            edgeObjects = imclose(edges1, strel('disk', round(nip.parameters.RemoveNeurites / 2), 0));
            ...
            edgeCellBodies = imopen(edgeObjects | nip.openedCellBodyMask, strel('disk', nip.parameters.RemoveNeurites, 0));
...
%            figure, imshow(double(cat(3,sobelNeurites, sobelNeurites,nip.secondNeuriteMask)));
            ...
            nip.neuriteExtensions = extendNeurites(nip, nip.secondNeuriteMask,...
                ...
                round(nip.parameters.RemoveNeurites/2));
...
%             nip.secondConnectedNeuriteMask = imreconstruct(nip.openedCellBodyMask, nip.secondCellMask) & nip.secondNeuriteMask;
        ...
        function status = resegmentNeuriteEdges(nip)
            ...
            thresh2 = nip.parameters.ImproveSegmentationQuality * graythresh(backgroundPixels);
            ...
            filledEdges2 = imclose(nip.secondEdges, strel('disk', floor(nip.parameters.RemoveNeurites / 2), 0));
...
%             edgeNeurites2 = filledEdges2 & ~occupiedECB;
            ...
            cellBody = imopen(occupiedECB | filledEdges2, strel('disk', nip.parameters.RemoveNeurites, 0));
...
%             bodies2 = imopen(filledEdges2 | nip.extendedCellBodyMask, strel('disk', nip.parameters.RemoveNeurites, 0));
...
%             occupiedECB = imreconstruct(nip.openedCellBodyMask, nip.extendedCellBodyMask);
% %             b = imopen(filledEdges2 | occupiedECB, strel('disk', nip.parameters.RemoveNeurites, 0));
...
%             figure, imshow(cat(3, r, g, b));
        ...
        function status = secondNeuriteResegmentation(nip)
           ...
           newRadius = floor(nip.parameters.RemoveNeurites / 2);
%            newRadius = nip.parameters.RemoveNeurites;
...
%            figure, imshow(thirdNeuriteMask);
        ...
        function status = closeNeuriteMask(nip)
            status = '';
            trueSquare = true(nip.parameters.ConnectNeuriteSections);
...
%nip.closedNeuriteMask = imclose(nip.secondNeuriteMask, strel('disk', nip.parameters.ConnectNeuriteSections, 0));
% Do two closings
nip.closedNeuriteMask = imclose(nip.closedNeuriteMask, strel('disk', nip.parameters.ConnectNeuriteSections, 0));
...
%             nip.graph.removeSpurs2(nip.parameters.RemoveNeurites);
