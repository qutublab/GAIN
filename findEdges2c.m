...
for i = 1:cc.NumObjects
    k = cc.PixelIdxList{i};
    neighbor1 = neighbor1IdxList{i};
    neighbor2 = neighbor2IdxList{i};
    k2 = zeros(numel(k) * 2, 1);
    [kr kc] = ind2sub(cc.ImageSize, k);
    [n1r n1c] = ind2sub(cc.ImageSize, neighbor1);
    [n2r n2c] = ind2sub(cc.ImageSize, neighbor2);
    idx = 1;
    for j = 1:numel(kr)
        for n =1:2
            if n == 1
            ...
            if nr ~= 0
                dist = abs(nr - kr(j)) + abs(nc - kc(j));
                if dist < 1 | dist > 2
                    error('i=%d  j=%d  n=%d dist=%d', i, j, n, dist)
                end
                k2(idx) = sub2ind(cc.ImageSize, nr, nc);
                idx = idx + 1;
            end
        end
    end
