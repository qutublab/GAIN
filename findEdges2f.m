...
[completeEdges incompleteEdges] = cellfun(@(pil, n1, n2, vim)findPath(pil, n1, n2, vim, cc.ImageSize), cc.PixelIdxList, neighbor1IdxList, neighbor2IdxList, localVertexIdMap, 'UniformOutput', false);



for i = 1:cc.NumObjects
    fprintf('i=%d (of %d)\n', i, cc.NumObjects);
    if numel(completeEdges{i}) == 1
        edgeCA = completeEdges{i};
    else
        edgeCA = incompleteEdges{i};
    end
    edge = edgeCA{1};
    for j = 1:(numel(edge.pathIdxList)-1)
        isSideAdjacent(edge.pathIdxList(j), edge.pathIdxList(j+1), cc.ImageSize);
    end
    pil = sort(edge.pathIdxList);
    if any(pil ~= cc.PixelIdxList{i})
        error('Object %d incorrectly processed', i)
    end
end
...
% incomplete edges
nextCA = cellfun(@(iceCA)cellfun(@(ice)-ice.vertices(2), iceCA), incompleteEdges, 'UniformOutput', false);
